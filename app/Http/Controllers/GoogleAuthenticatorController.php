...
class GoogleAuthenticatorController extends Controller
{
	...
	public function google_aoth( $secret_code = null )
	{

	    $googleAuth = array();

	    /**************** TO GENERATE GOOGLE AUTH CODE START ****************/
	    ...
	    if (empty($secret_code)) {
	        ...
	        // $secret = $data['secret'] = 'GTG6VJIPLSEZFNQ6';
	    }else{
	        $secret = $secret_code;
	    }
	    // print_r($secret);exit;
	    ...
	    // $data['qrCodeUrl'] = $this->getQRCodeGoogleUrl('STEP', 'test_prasanth@gmail.com', $secret);
	    $qrCodeUrl = $this->getQRCodeGoogleUrl(config('app.name'), Auth::user()->name, $secret);

	    // also, you can get a code to test the service
	    $oneCode = $this->getCode($secret);
	    /**************** TO GENERATE GOOGLE AUTH CODE END ****************/
	    $googleAuth = [
	        'secret'        => $secret,
